...
static volatile struct __attribute__((__packed__)) {
	...
	uint32_t maximum_current_capabilities;
} *sdhost;
...
do {							\
	for (uint32_t i = 0; i < usec && !(stop_if_true); i++)\
...
static uint32_t sd_get_clock_divider(uint32_t base_clock, uint32_t target_rate)
{
    ...
    {
    	uint32_t mod;
        targetted_divisor = unsigned_divmod(base_clock, target_rate, &mod);
        if(mod)
            targetted_divisor--;
    }
    ...
    int actual_clock = unsigned_divmod(base_clock, denominator, NULL);
    uart0_printf("EMMC: base_clock: %d, target_rate: %d, divisor: %08x, "
...
static int sd_switch_clock_rate(uint32_t base_clock, uint32_t target_rate)
{
    ...
    {
        uart0_printf("EMMC: couldn't get a valid divider for target rate %d Hz\n",
               target_rate);
        return -1;
    }

    // Wait for the command inhibit (CMD and DAT) bits to clear
    while(GET32(emmc_base + EMMC_STATUS) & 0x3)
        usleep(1000);
...
#ifdef EMMC_DEBUG
    uart0_printf("EMMC: successfully set clock rate to %d Hz\n", target_rate);
#endif
    return 0;
}
...
static int sd_reset_cmd()
{
	...
	{
		uart0_printf("EMMC: CMD line did not reset properly\n");
		return -1;
	}
	return 0;
}
...
static int sd_reset_dat()
{
	...
	{
		uart0_printf("EMMC: DAT line did not reset properly\n");
		return -1;
	}
	return 0;
}
...
static void sd_issue_command_begin(uint32_t cmd_reg, uint32_t argument, uint16_t block_size, uint16_t block_count) {
	// This is as per HCSS 3.7.1.1/3.7.2.2

    // Check Command Inhibit
    while(sdhost->status & 0x1);
    ...
    if((cmd_reg & SD_CMD_RSPNS_TYPE_MASK) == SD_CMD_RSPNS_TYPE_48B)
    {
        ...
        if((cmd_reg & SD_CMD_TYPE_MASK) != SD_CMD_TYPE_ABORT)
        {
            ...
            while(sdhost->status & 0x2);
        }
    }

    sdhost->block_size = block_size;
    sdhost->block_count = block_count;
    sdhost->arg = argument;
    sdhost->command = cmd_reg;
}


static bool sd_issue_command_complete(struct emmc_block_dev *dev, uint32_t cmd_reg) {

	/* wait for interrupts */
	for (int cntr = 0; cntr < 10000; cntr++)
    	if (sdhost->normal_interrupt_status & 0x8001)
    		break;
    uint32_t error = ((uint32_t) sdhost->error_interrupt_status) << 16;
    bool success = (sdhost->normal_interrupt_status & 0x1) &&
    			   (!sdhost->error_interrupt_status);
    sdhost->normal_interrupt_status = 0x1;
    if (!success)
    {
    ...
    switch(cmd_reg & SD_CMD_RSPNS_TYPE_MASK)
    {
        ...
        case SD_CMD_RSPNS_TYPE_48B:
            dev->last_r0 = sdhost->resp[0];
            break;

        case SD_CMD_RSPNS_TYPE_136:
            dev->last_r0 = sdhost->resp[0];
            dev->last_r1 = sdhost->resp[1];
            dev->last_r2 = sdhost->resp[2];
            dev->last_r3 = sdhost->resp[3];
            break;
    }
...
static void sd_issue_command_int(struct emmc_block_dev *dev, uint32_t cmd_reg, uint32_t argument, useconds_t timeout)
{
    ...
    sd_issue_command_begin(cmd_reg, argument, dev->block_size, dev->blocks_to_transfer);
    if (!sd_issue_command_complete(dev, cmd_reg))
    	return;
    ...
    if((cmd_reg & SD_CMD_ISDATA))
    {
        ...
        while(cur_block < dev->blocks_to_transfer)
        {
			...
			if(dev->blocks_to_transfer > 1)
				uart0_printf("SD: multi block transfer, awaiting block %d ready\n",
				cur_block);
#endif
            ...
            {
#ifdef EMMC_DEBUG
            uart0_printf("SD: error occured whilst waiting for data ready interrupt\n");
#endif
            ...
            while(cur_byte_no < dev->block_size)
            {
                if(is_write)
				{
                    ...
                    PUT32(emmc_base + EMMC_DATA, data);
				}
                else
				{
					uint32_t data = GET32(emmc_base + EMMC_DATA);
...
#ifdef EMMC_DEBUG
			uart0_printf("SD: block %d transfer complete\n", cur_block);
#endif
       ...
       (cmd_reg & SD_CMD_ISDATA)))
    {
        ...
        else
        {
            ...
            {
#ifdef EMMC_DEBUG
                uart0_printf("SD: error occured whilst waiting for transfer complete interrupt\n");
#endif
                dev->last_error = irpts & 0xffff0000;
                dev->last_interrupt = irpts;
                return;
...
static void sd_handle_card_interrupt(struct emmc_block_dev *dev)
{
    ...
    uart0_printf("SD: card interrupt\n");
    uart0_printf("SD: controller status: %08x\n", status);
#endif
    ...
    if(dev->card_rca)
    {
        ...
        {
#ifdef EMMC_DEBUG
            uart0_printf("SD: unable to get card status\n");
#endif
        }
        else
        {
#ifdef EMMC_DEBUG
            uart0_printf("SD: card status: %08x\n", dev->last_r0);
#endif
        }
    }
    else
    {
#ifdef EMMC_DEBUG
        uart0_printf("SD: no card currently selected\n");
#endif
    }
}
...
static void sd_handle_interrupts(struct emmc_block_dev *dev)
{
    ...
    {
#ifdef EMMC_DEBUG
        uart0_printf("SD: spurious command complete interrupt\n");
#endif
    ...
    {
#ifdef EMMC_DEBUG
        uart0_printf("SD: spurious transfer complete interrupt\n");
#endif
    ...
    {
#ifdef EMMC_DEBUG
        uart0_printf("SD: spurious block gap event interrupt\n");
#endif
    ...
    {
#ifdef EMMC_DEBUG
        uart0_printf("SD: spurious DMA interrupt\n");
#endif
    ...
    {
#ifdef EMMC_DEBUG
        uart0_printf("SD: spurious buffer write ready interrupt\n");
#endif
    ...
    {
#ifdef EMMC_DEBUG
        uart0_printf("SD: spurious buffer read ready interrupt\n");
#endif
    ...
    {
#ifdef EMMC_DEBUG
        uart0_printf("SD: card insertion detected\n");
#endif
    ...
    {
#ifdef EMMC_DEBUG
        uart0_printf("SD: card removal detected\n");
#endif
    ...
    {
#ifdef EMMC_DEBUG
        uart0_printf("SD: card interrupt detected\n");
#endif
    ...
    {
#ifdef EMMC_DEBUG
        uart0_printf("SD: spurious error interrupt: %08x\n", irpts);
#endif
...
static void sd_issue_command(struct emmc_block_dev *dev, uint32_t command, uint32_t argument, useconds_t timeout)
{
    ...
    if(command & IS_APP_CMD)
    {
        command &= 0xff;
#ifdef EMMC_DEBUG
        uart0_printf("SD: issuing command ACMD%d\n", command);
#endif

        if(sd_acommands[command] == SD_CMD_RESERVED(0))
        {
            uart0_printf("SD: invalid command ACMD%d\n", command);
            dev->last_cmd_success = 0;
            return;
        }
    ...
    {
#ifdef EMMC_DEBUG
        uart0_printf("SD: issuing command CMD%d\n", command);
#endif

        if(sd_commands[command] == SD_CMD_RESERVED(0))
        {
            uart0_printf("SD: invalid command CMD%d\n", command);
            dev->last_cmd_success = 0;
            return;
        }
    ...
    if(FAIL(dev))
    {
        uart0_printf("SD: error issuing command: interrupts %08x: ", dev->last_interrupt);
        if(dev->last_error == 0)
            uart0_printf("TIMEOUT");
        else
        {
        ...
        uart0_printf("SD: command completed successfully\n");
#endif
}

int sd_card_init(struct block_device **dev)
{
	sdhost = (void *) TranslateAddr(emmc_base);

	// Power cycle the card to ensure its in its startup state
	if(bcm_2708_power_cycle() != 0)
	{
		uart0_printf("EMMC: BCM2708 controller did not power cycle successfully\n");
	}
...
#ifdef EMMC_DEBUG
	uart0_printf("EMMC: resetting controller\n");
#endif
	uint32_t control1 = GET32(emmc_base + EMMC_CONTROL1);
	control1 |= (1 << 24);
	// Disable clock
	...
	{
		uart0_printf("EMMC: controller did not reset properly\n");
		return -1;
	}
#ifdef EMMC_DEBUG
	uart0_printf("EMMC: control0: %08x, control1: %08x, control2: %08x\n",
...
#ifdef EMMC_DEBUG
	uart0_printf("EMMC: capabilities: %08x%08x\n", capabilities_1, capabilities_0);
#endif

	// Check for a valid card
#ifdef EMMC_DEBUG
	uart0_printf("EMMC: checking for an inserted card\n");
#endif
    TIMEOUT_WAIT(GET32(emmc_base + EMMC_STATUS) & (1 << 16), 500000);
	uint32_t status_reg = GET32(emmc_base + EMMC_STATUS);
	if((status_reg & (1 << 16)) == 0)
	{
		uart0_printf("EMMC: no card inserted\n");
		return -1;
	}
#ifdef EMMC_DEBUG
	uart0_printf("EMMC: status: %08x\n", status_reg);
#endif
	...
	if(base_clock == 0)
	{
	    uart0_printf("EMMC: assuming clock rate to be 100MHz\n");
	    base_clock = 100000000;
	}

	// Set clock rate to something slow
#ifdef EMMC_DEBUG
	uart0_printf("EMMC: setting clock rate\n");
#endif
	...
	{
		uart0_printf("EMMC: unable to get a valid clock divider for ID frequency\n");
		return -1;
	}
	control1 |= f_id;
	...
	{
		uart0_printf("EMMC: controller's clock did not stabilise within 1 second\n");
		return -1;
	}
#ifdef EMMC_DEBUG
	uart0_printf("EMMC: control0: %08x, control1: %08x\n",
			GET32(emmc_base + EMMC_CONTROL0),
			GET32(emmc_base + EMMC_CONTROL1));
#endif
	...
	// Enable the SD clock
#ifdef EMMC_DEBUG
	uart0_printf("EMMC: enabling SD clock\n");
#endif
	usleep(2000);
	control1 = GET32(emmc_base + EMMC_CONTROL1);
	control1 |= 4;
	...
	usleep(2000);
#ifdef EMMC_DEBUG
	uart0_printf("EMMC: SD clock enabled\n");
#endif
...
#ifdef EMMC_DEBUG
	uart0_printf("EMMC: interrupts disabled\n");
#endif
	usleep(2000);
...
#ifdef EMMC_DEBUG
	uart0_printf("EMMC: device structure created\n");
#endif
	...
	if(FAIL(ret))
	{
        uart0_printf("SD: no CMD0 response\n");
        return -1;
	}
...
#ifdef EMMC_DEBUG
    uart0_printf("SD: note a timeout error on the following command (CMD8) is normal "
    ...
    {
        uart0_printf("SD: failure sending CMD8 (%08x)\n", ret->last_interrupt);
        return -1;
    }
    else
    {
        ...
        {
            uart0_printf("SD: unusable card\n");
#ifdef EMMC_DEBUG
            uart0_printf("SD: CMD8 response %08x\n", ret->last_r0);
#endif
            return -1;
        }
        else
...
#ifdef EMMC_DEBUG
    uart0_printf("SD: note that a timeout error on the following command (CMD5) is "
    ...
    if(!TIMEOUT(ret))
    {
        ...
        {
            uart0_printf("SD: SDIO card detected - not currently supported\n");
#ifdef EMMC_DEBUG
            uart0_printf("SD: CMD5 returned %08x\n", ret->last_r0);
#endif
            return -1;
        }
    }
...
#ifdef EMMC_DEBUG
    uart0_printf("SD: sending inquiry ACMD41\n");
#endif
    sd_issue_command(ret, ACMD(41), 0, 500000);
    if(FAIL(ret))
    {
        uart0_printf("SD: inquiry ACMD41 failed\n");
        return -1;
    }
#ifdef EMMC_DEBUG
    uart0_printf("SD: inquiry ACMD41 returned %08x\n", ret->last_r0);
#endif
	...
	while(card_is_busy)
	{
	    ...
	    if(FAIL(ret))
	    {
	        uart0_printf("SD: error issuing ACMD41\n");
	        return -1;
	    }
	    ...
	    else
	    {
	        // Card is still busy
#ifdef EMMC_DEBUG
            uart0_printf("SD: card is busy, retrying\n");
#endif
            usleep(5000);
	    }
	}
...
#ifdef EMMC_DEBUG
	uart0_printf("SD: card identified: OCR: %04x, 1.8v support: %d, SDHC support: %d\n",
	...
	{
#ifdef EMMC_DEBUG
        uart0_printf("SD: switching to 1.8V mode\n");
#endif
	    // As per HCSS 3.6.1
	    ...
	    {
#ifdef EMMC_DEBUG
            uart0_printf("SD: error issuing VOLTAGE_SWITCH\n");
#endif
	    ...
	    {
#ifdef EMMC_DEBUG
            uart0_printf("SD: DAT[3:0] did not settle to 0\n");
#endif
	        ret->failed_voltage_switch = 1;
			sd_power_off();
	        return sd_card_init((struct block_device **)&ret);
	    ...
	    {
#ifdef EMMC_DEBUG
            uart0_printf("SD: controller did not keep 1.8V signal enable high\n");
#endif
	        ret->failed_voltage_switch = 1;
			sd_power_off();
	        return sd_card_init((struct block_device **)&ret);
	    ...
	    {
#ifdef EMMC_DEBUG
            uart0_printf("SD: DAT[3:0] did not settle to 1111b (%01x)\n", dat30);
#endif
	        ret->failed_voltage_switch = 1;
			sd_power_off();
	        return sd_card_init((struct block_device **)&ret);
...
#ifdef EMMC_DEBUG
        uart0_printf("SD: voltage switch complete\n");
#endif
	}
	...
	if(FAIL(ret))
	{
	    uart0_printf("SD: error sending ALL_SEND_CID\n");
	    return -1;
	}
...
#ifdef EMMC_DEBUG
	uart0_printf("SD: card CID: %08x%08x%08x%08x\n", card_cid_3, card_cid_2, card_cid_1, card_cid_0);
#endif
	uint32_t *dev_id = (uint32_t *) ret->bd.device_id;
	dev_id[0] = card_cid_0;
	dev_id[1] = card_cid_1;
	dev_id[2] = card_cid_2;
	dev_id[3] = card_cid_3;
	...
	if(FAIL(ret))
    {
        uart0_printf("SD: error sending SEND_RELATIVE_ADDR\n");
        return -1;
    }

	uint32_t cmd3_resp = ret->last_r0;
#ifdef EMMC_DEBUG
	uart0_printf("SD: CMD3 response: %08x\n", cmd3_resp);
#endif
	...
	if(crc_error)
	{
		uart0_printf("SD: CRC error\n");
		return -1;
	}

	if(illegal_cmd)
	{
		uart0_printf("SD: illegal command\n");
		return -1;
	}

	if(error)
	{
		uart0_printf("SD: generic error\n");
		return -1;
	}

	if(!ready)
	{
		uart0_printf("SD: not ready for data\n");
		return -1;
	}

#ifdef EMMC_DEBUG
	uart0_printf("SD: RCA: %04x\n", ret->card_rca);
#endif
	...
	if(FAIL(ret))
	{
	    uart0_printf("SD: error sending CMD7\n");
	    return -1;
	}
	...
	{
		uart0_printf("SD: invalid status (%d)\n", status);
		return -1;
	}
	...
	if(!ret->card_supports_sdhc)
	{
	    ...
	    if(FAIL(ret))
	    {
	        uart0_printf("SD: error sending SET_BLOCKLEN\n");
	        return -1;
	    }
	}
	ret->block_size = 512;
	...
	if(FAIL(ret))
	{
	    uart0_printf("SD: error sending SEND_SCR\n");
	    return -1;
	}
...
#ifdef EMMC_DEBUG
    uart0_printf("SD: &scr: %08x\n", &ret->scr.scr[0]);
    uart0_printf("SD: SCR[0]: %08x, SCR[1]: %08x\n", ret->scr.scr[0], ret->scr.scr[1]);;
    uart0_printf("SD: SCR: %08x%08x\n", byte_swap(ret->scr.scr[0]), byte_swap(ret->scr.scr[1]));
    uart0_printf("SD: SCR: version %s, bus_widths %01x\n", sd_versions[ret->scr.sd_version],
    ...
    if(ret->scr.sd_bus_widths & 0x4)
    {
...
#ifdef SD_4BIT_DATA
#ifdef EMMC_DEBUG
        uart0_printf("SD: switching to 4-bit data mode\n");
#endif
        ...
        if(FAIL(ret))
            uart0_printf("SD: switch to 4-bit data mode failed\n");
        else
        {
...
#ifdef EMMC_DEBUG
                uart0_printf("SD: switch to 4-bit complete\n");
#endif
        }
#endif
    }

	uart0_printf("SD: found a valid version %s SD card\n", sd_versions[ret->scr.sd_version]);
#ifdef EMMC_DEBUG
	uart0_printf("SD: setup successful (status %d)\n", status);
#endif
...
static int sd_ensure_data_mode(struct emmc_block_dev *edev)
{
...
#ifdef EMMC_DEBUG
	uart0_printf("SD: ensure_data_mode() obtaining status register for card_rca %08x: ",
		edev->card_rca);
#endif
    ...
    if(FAIL(edev))
    {
        uart0_printf("SD: ensure_data_mode() error sending CMD13\n");
        edev->card_rca = 0;
        return -1;
    }
	...
	if(cur_state == 3)
	{
		...
		if(FAIL(edev))
		{
			uart0_printf("SD: ensure_data_mode() no response from CMD17\n");
			edev->card_rca = 0;
			return -1;
		}
	}
	else if(cur_state == 5)
	{
		...
		if(FAIL(edev))
		{
			uart0_printf("SD: ensure_data_mode() no response from CMD12\n");
			edev->card_rca = 0;
			return -1;
		}
	...
	{
#ifdef EMMC_DEBUG
		uart0_printf("SD: ensure_data_mode() rechecking status: ");
#endif
        sd_issue_command(edev, SEND_STATUS, edev->card_rca << 16, 500000);
        if(FAIL(edev))
		{
			uart0_printf("SD: ensure_data_mode() no response from CMD13\n");
			edev->card_rca = 0;
			return -1;
		}
		status = edev->last_r0;
		...
		if(cur_state != 4)
		{
			uart0_printf("SD: unable to initialise SD card to "
					"data mode (state %d)\n", cur_state);
			edev->card_rca = 0;
			return -1;
		}
...
static int sd_do_data_command(struct emmc_block_dev *edev, int is_write, uint8_t *buf, size_t buf_size, uint32_t block_no)
{
	...
	{
	    uart0_printf("SD: do_data_command() called with buffer size (%d) less than "
            "block size (%d)\n", buf_size, edev->block_size);
        return -1;
	}

	edev->blocks_to_transfer = unsigned_divmod(buf_size, edev->block_size, &mod);
	if (mod)
	{
	    uart0_printf("SD: do_data_command() called with buffer size (%d) not an "
	...
	while(retry_count < max_retries)
	{
        ...
        {
            uart0_printf("SD: error sending CMD%d, ", command);
            uart0_printf("error = %08x.  ", edev->last_error);
            retry_count++;
            if(retry_count < max_retries)
                uart0_printf("Retrying...\n");
            else
                uart0_printf("Giving up.\n");
        }
	}
	if(retry_count == max_retries)
    {
...
#ifdef EMMC_DEBUG
	uart0_printf("SD: read() card ready, reading from block %u\n", block_no);
#endif
...
#ifdef EMMC_DEBUG
	uart0_printf("SD: data read successful\n");
#endif
...
#ifdef EMMC_DEBUG
	uart0_printf("SD: write() card ready, reading from block %u\n", block_no);
#endif
...
#ifdef EMMC_DEBUG
	uart0_printf("SD: write read successful\n");
#endif
